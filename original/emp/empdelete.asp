...
if Request.QueryString("DeleteIt")="Delete" then
	if Request.QueryString("chk_emp").Count >0 then
		For Dummy=1 to Request.QueryString("chk_emp").Count
			vStr =  vStr + Request.QueryString("chk_emp")(Dummy) + ", " 
		Next
		vFinalStr = Left(vStr, (Len(vStr)-2))
		vQuery ="delete from employees where emp_id in (" + trim(vFinalStr) + ")"
		cn.Execute vQuery
...
if Request.QueryString("UpdateIt")="Update" then
 	if Request.QueryString("radio_emp").Count > 0 then
 		vUpdateStr = "empupdate.asp?emp_id=" + Request.QueryString("radio_emp")
